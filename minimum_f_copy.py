...
def free_energy (AL, rho, matrix):
    ...
    # todo
    cost = (2*pi/m) * np.dot(AL,np.log(rho * AL).T) + (2 * pi**2 * rho/m**2) * np.dot (np.dot(AL, matrix), AL.T) + (np.sum(AL * dx) - 1)**2
...
def L_model_backward_free_e(AL, caches, rho, matrix):
    ...
    dAL = np.log (AL + 1) + (2* pi * rho/m) * np.dot(AL, matrix) 
...
def model_optimize_free_e (X, rho, matrix, input_parameter, layers_dims, optimizer, learning_rate = 0.0007, beta = 0.9,
    ...
    parameters = copy.deepcopy(input_parameter)
...
def model_optimize_free_e_without_guess (X, rho, matrix, layers_dims, optimizer, learning_rate = 0.0007, beta = 0.9,
          beta1 = 0.9, beta2 = 0.999,  epsilon = 1e-8, num_epochs = 10000, print_cost = True):
